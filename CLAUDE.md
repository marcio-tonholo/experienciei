...
- **`payments` table** — one row per booking (`booking_id` unique FK), tracks `status` (`pendente | pago | falhou | expirado`) and a separate `repasse_status` (`pendente | repassado`) for mentor payout bookkeeping (admin-only, no payout automation yet). Clients get `select`/`insert`/`update` grants, but there are no client-facing INSERT/general-UPDATE RLS policies — rows are only ever written by the edge functions using the service-role key, which bypasses RLS. Students/mentors can only SELECT their own; admins can SELECT/UPDATE all (e.g. to mark `repasse_status = 'repassado'`).
