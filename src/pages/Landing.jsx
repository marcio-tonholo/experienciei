...
const Logo = ({ white }) => (
  <div className="flex items-center gap-2">
    <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${white ? 'bg-white/20' : 'bg-[#1E3A8A]'}`}>
    ...
    <span className={`text-xl font-bold ${white ? 'text-white' : 'text-[#1E3A8A]'}`}>Experenciei</span>
  </div>
)
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-[#E2E8F0]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {['Como Funciona', 'Para Médicos', 'Para Estudantes', 'Especialidades'].map((item) => (
              <a key={item} href="#" className="text-sm font-medium text-[#374151] hover:text-[#1E3A8A] transition-colors">
                {item}
              </a>
            ))}
          </nav>
        ...
        {menuOpen && (
          <div className="md:hidden bg-white border-t border-[#E2E8F0] px-4 py-4 space-y-3">
            {['Como Funciona', 'Para Médicos', 'Para Estudantes', 'Especialidades'].map((item) => (
              <a key={item} href="#" className="block text-sm font-medium text-[#374151] py-1">
                {item}
              </a>
            ))}
            {user ? (
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight mb-4">
                Conecte-se com os{' '}
                <span className="text-[#93C5FD]">Melhores</span>{' '}
                Cirurgiões do Brasil
              </h1>
              <p className="text-blue-100 text-base sm:text-lg mb-8 leading-relaxed">
                Experiência cirúrgica real através de mentorias práticas com especialistas renomados.
                Aprenda, observe e evolua sua carreira médica.
              </p>
              ...
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6 sm:p-8 w-full max-w-sm">
                ...
                <div className="flex items-center gap-4 mb-6 pb-6 border-b border-white/20">
                  ...
                  <div>
                    ...
                    <p className="text-blue-200 text-sm">Cirurgião Cardiovascular</p>
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  {[['500+', 'Médicos'], ['1.200+', 'Mentorias'], ['50+', 'Especialidades']].map(([num, label]) => (
                    <div key={label}>
      ...
      <section className="py-16 md:py-24 bg-[#F8FAFC]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#0F172A] mb-3">Como o Experenciei Funciona</h2>
          ...
          <div className="grid sm:grid-cols-3 gap-8">
            {[
              ...
              <div key={step.num} className="bg-white rounded-2xl p-7 shadow-sm border border-[#E2E8F0] relative">
      ...
      <section className="py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#0F172A] mb-3">Especialidades Disponíveis</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {specialties.map((sp) => (
              ...
              >
                ...
                <p className="font-semibold text-[#1E293B] text-sm group-hover:text-[#1E3A8A]">{sp.name}</p>
                <p className="text-[#9CA3AF] text-xs mt-1">{sp.count} especialistas</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-[#F8FAFC]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#0F172A] mb-3">O Que Dizem Nossos Usuários</h2>
          </div>
          <div className="grid sm:grid-cols-3 gap-6">
            {testimonials.map((t) => (
              <div key={t.name} className="bg-white rounded-2xl p-6 shadow-sm border border-[#E2E8F0]">
      ...
      <section className="py-16 md:py-24 bg-gradient-to-br from-[#1E3A8A] to-[#2563EB] text-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold mb-4">Pronto para Transformar sua Carreira Médica?</h2>
          <p className="text-blue-100 mb-8">
            Junte-se a centenas de médicos que já estão evoluindo suas carreiras através de mentorias práticas.
          </p>
          <button
            onClick={() => navigate('/auth')}
          ...
          <p className="text-blue-200 text-sm mt-3">Sem cartão de crédito necessário</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            ...
            <div>
              ...
              {['Como Funciona', 'Para Médicos', 'Para Estudantes', 'Sobre Nós'].map(l => (
                <a key={l} href="#" className="block text-[#94A3B8] text-sm hover:text-white mb-2">{l}</a>
              ))}
            </div>
            <div>
              <h4 className="font-semibold mb-3 text-sm">Especialidades</h4>
              {['Cardiologia', 'Neurologia', 'Ortopedia', 'Ver Todas'].map(l => (
                <a key={l} href="#" className="block text-[#94A3B8] text-sm hover:text-white mb-2">{l}</a>
              ))}
            </div>
            <div>
              <h4 className="font-semibold mb-3 text-sm">Contato</h4>
              <p className="text-[#94A3B8] text-sm">contato@experenciei.com.br</p>
            </div>
          </div>
