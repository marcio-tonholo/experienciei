...
export default function Explorar() {
  ...
  useEffect(() => {
    ...
    async function loadOfferings() {
      ...
      if (data) {
        setOfferings(data)
        setSpecialties([...new Set(data.map(o => o.procedures?.especialidade).filter(Boolean))])
      }
      setLoading(false)
    }
    loadOfferings()
  ...
  async function handleApply(offeringId, mentorId) {
    ...
    const { data, error } = await supabase
      ...
      .select('offering_id, status')
      .single()
    if (!error && data) setBookings(prev => {
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <section className="bg-gradient-to-br from-[#1E3A8A] to-[#2563EB] py-10 md:py-14">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <h1 className="text-white text-2xl sm:text-3xl font-bold mb-2">Explorar Mentorias</h1>
          <p className="text-blue-200 text-sm mb-8">Vagas abertas com especialistas de todo o Brasil</p>
          <div className="bg-white rounded-2xl p-3 shadow-xl flex gap-2">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="px-3 py-2.5 text-[#64748B] hover:text-[#1E293B] text-sm flex-shrink-0">
                ✕
              </button>
            )}
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 pb-16">
        ...
        {!loading && (
          <p className="text-sm text-[#64748B] mb-5">
            {filtered.length} {filtered.length === 1 ? 'mentoria disponível' : 'mentorias disponíveis'}
        ...
        {!authLoading && !isAluno && (
          <div className={`flex items-center gap-3 rounded-2xl px-4 py-3 mb-6 text-sm ${
            !user
            ...
            {!user
              ? <span><strong>Crie uma conta gratuita</strong> para se candidatar a qualquer mentoria.</span>
              : <span>Você está navegando como <strong>mentor</strong>. Explore as mentorias disponíveis na plataforma.</span>
            }
            {!user && (
              <button
                onClick={() => navigate('/auth')}
        ...
        {loading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(o => {
              ...
              const existingBooking = bookings.find(b => b.offering_id === o.id)
              ...
              } else if (existingBooking && existingBooking.status !== 'cancelado') {
                actionBtn = (
