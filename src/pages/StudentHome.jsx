...
const COMPLEXITY = {
  basico:        { label: 'Básico',        color: 'bg-blue-50 text-blue-600',     strip: 'bg-blue-50 border-b border-blue-100 text-blue-600'      },
  intermediario: { label: 'Intermediário', color: 'bg-yellow-50 text-yellow-700', strip: 'bg-yellow-50 border-b border-yellow-100 text-yellow-700' },
  avancado:      { label: 'Avançado',      color: 'bg-purple-50 text-purple-700', strip: 'bg-purple-50 border-b border-purple-100 text-purple-700' },
}
...
function EditStudentProfileModal({ profileId, initial, studentInitial, onClose, onSaved }) {
  ...
  function addObj(raw) {
    const v = raw.trim()
    if (!v || form.objetivos.includes(v)) { setObjInput(''); return }
    set('objetivos', [...form.objetivos, v])
    setObjInput('')
  }

  function removeObj(i) { set('objetivos', form.objetivos.filter((_, idx) => idx !== i)) }
  ...
  async function handleSave() {
    ...
    const [r1, r2] = await Promise.all([
      supabase.from('profiles').update({ nome: form.nome.trim(), cidade: form.cidade.trim() }).eq('id', profileId),
      supabase.from('student_profiles').update({
        nivel:         form.nivel,
        especialidade: form.especialidade.trim() || null,
        crm:           form.crm.trim() || null,
        ano_formacao:  form.ano_formacao !== '' ? Number(form.ano_formacao) : null,
  ...
  const inp = 'w-full border border-[#E2E8F0] rounded-xl px-3 py-2.5 text-sm text-[#0F172A] bg-white focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB]'
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[92vh] flex flex-col">
        ...
        <div className="overflow-y-auto px-5 py-5 space-y-4 flex-1">
          <div>
            <label className="text-xs font-semibold text-[#374151] block mb-1.5">Nome completo</label>
            <input type="text" value={form.nome} onChange={e => set('nome', e.target.value)} className={inp} />
          </div>
          <div>
            <label className="text-xs font-semibold text-[#374151] block mb-1.5">Cidade</label>
            <input type="text" value={form.cidade} onChange={e => set('cidade', e.target.value)} placeholder="São Paulo, SP" className={inp} />
          </div>
          <div>
            <label className="text-xs font-semibold text-[#374151] block mb-1.5">Nível</label>
            <select value={form.nivel} onChange={e => set('nivel', e.target.value)} className={inp}>
          ...
          <div>
            <label className="text-xs font-semibold text-[#374151] block mb-1.5">Especialidade</label>
            <input type="text" value={form.especialidade} onChange={e => set('especialidade', e.target.value)} className={inp} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-[#374151] block mb-1.5">CRM (opcional)</label>
              <input type="text" value={form.crm} onChange={e => set('crm', e.target.value)} className={inp} />
            </div>
            <div>
              <label className="text-xs font-semibold text-[#374151] block mb-1.5">Ano de formação</label>
              <input type="number" min={1980} max={2030} value={form.ano_formacao} onChange={e => set('ano_formacao', e.target.value)} className={inp} />
            </div>
          </div>
          <div>
            <label className="text-xs font-semibold text-[#374151] block mb-1.5">Objetivos</label>
            <div className="flex flex-wrap gap-1.5 mb-2">
              {form.objetivos.map((o, i) => (
                <span key={i} className="flex items-center gap-1 text-xs bg-[#EFF6FF] text-[#1E3A8A] px-2.5 py-1 rounded-full">
                  {o}
                  <button type="button" onClick={() => removeObj(i)} className="text-[#93C5FD] hover:text-[#1E3A8A] leading-none text-base">×</button>
                </span>
              ))}
            </div>
            <input
              type="text" value={objInput}
              onChange={e => setObjInput(e.target.value)}
              onKeyDown={e => {
                if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addObj(objInput) }
                if (e.key === 'Backspace' && !objInput && form.objetivos.length) removeObj(form.objetivos.length - 1)
              }}
        ...
        <div className="px-5 py-4 border-t border-[#E2E8F0] flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-[#E2E8F0] text-[#374151] rounded-xl text-sm font-medium hover:bg-[#F8FAFC] transition-colors">Cancelar</button>
          <button onClick={handleSave} disabled={saving} className="flex-[2] py-2.5 bg-gradient-to-r from-[#1E3A8A] to-[#2563EB] text-white rounded-xl text-sm font-semibold hover:opacity-90 disabled:opacity-50 transition-opacity">
...
function CertificateModal({ booking, studentName, onClose }) {
  ...
  const dateStr      = o?.inicio
    ? new Date(o.inicio).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })
    : ''
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[#E2E8F0]">
          <h2 className="text-base font-bold text-[#0F172A]">Certificado de Participação</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-full hover:bg-[#F1F5F9] flex items-center justify-center text-[#64748B] text-lg leading-none">✕</button>
        </div>

        <div className="overflow-y-auto flex-1 p-5">
          <div className="border-4 border-[#1E3A8A] p-8 relative bg-white select-none" style={{ fontFamily: 'Georgia, serif' }}>
            ...
            <div className="flex items-center justify-center gap-2.5 mb-5">
              ...
              <span className="text-xl font-bold text-[#1E3A8A]" style={{ fontFamily: 'Arial, sans-serif' }}>Experenciei</span>
            </div>

            <p className="text-center text-[10px] tracking-[4px] uppercase text-[#94A3B8] mb-1" style={{ fontFamily: 'Arial, sans-serif' }}>
              Plataforma de Mentoria Cirúrgica
            </p>
            <h1 className="text-center text-2xl font-bold text-[#1E3A8A] mb-5">Certificado de Participação</h1>

            <div className="h-0.5 mx-auto mb-5" style={{ width: '75%', background: 'linear-gradient(to right, transparent, #1E3A8A, transparent)' }} />

            <p className="text-center text-[#0F172A] leading-loose mb-5">
              Certificamos que<br />
              <span className="text-[#1E3A8A] font-bold text-lg">{studentName}</span><br />
              participou da experiência cirúrgica<br />
              <span className="text-[#1E3A8A] font-bold">{procNome}</span>
              {especialidade && (
                <><br /><span className="text-xs text-[#64748B]" style={{ fontFamily: 'Arial, sans-serif' }}>{especialidade}</span></>
              )}
              <br />
              realizada em <strong>{dateStr}</strong>{cidade && <>, em <strong>{cidade}</strong></>}
              <br />
              sob orientação de <span className="text-[#1E3A8A] font-bold">{mentorNome}</span>
            </p>

            <div className="h-0.5 mx-auto mb-4" style={{ width: '75%', background: 'linear-gradient(to right, transparent, #1E3A8A, transparent)' }} />

            <div className="text-center" style={{ fontFamily: 'Arial, sans-serif' }}>
              <span className="text-xs bg-[#EFF6FF] text-[#1E3A8A] border border-[#BFDBFE] px-3 py-1 rounded-full">ID: {certId}</span>
              <p className="text-xs text-[#94A3B8] mt-2">Emitido em {emitido}</p>
            </div>
          </div>
        </div>

        <div className="px-5 py-4 border-t border-[#E2E8F0] flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-[#E2E8F0] text-[#374151] rounded-xl text-sm font-medium hover:bg-[#F8FAFC] transition-colors">
            Fechar
          </button>
          <button onClick={handlePrint} className="flex-[2] py-2.5 bg-gradient-to-r from-[#1E3A8A] to-[#2563EB] text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity">
            Gerar PDF / Imprimir
          </button>
        </div>
      </div>
...
export default function StudentHome() {
  ...
  const displayNivel  = profileOverride.nivel  ?? studentProfile.nivel ?? profile?.nivel
  const initials    = displayNome.split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase() || 'EU'
  const firstName   = displayNome.split(' ')[0] || 'Você'
  const nivelLabel  = { estudante: 'Estudante', residente: 'Residente', especialista: 'Especialista' }[displayNivel] || 'Aluno'

  const specialties = [...new Set(offerings.map(o => o.procedures?.especialidade).filter(Boolean))]
  const filteredOfferings = offerings.filter(o => {
    if (filterSpec !== 'Todos' && o.procedures?.especialidade !== filterSpec) return false
    if (searchQuery.trim()) {
  ...
  async function loadOfferings() {
    ...
    const { data } = await supabase
      .from('offerings')
      .select('*, procedures(nome, especialidade, complexidade), profiles(nome)')
      .eq('status', 'publicado')
  ...
  async function loadBookings() {
    ...
    const { data } = await supabase
      .from('bookings')
      .select('*, mentor:profiles!mentor_id(nome), offerings(id, titulo, inicio, fim, preco, cidade, local_descricao, status, procedures(nome, especialidade))')
  ...
  async function handleApply(offeringId, mentorId) {
    ...
    const { data, error } = await supabase
      .from('bookings')
      .upsert(
        { aluno_id: profile.id, mentor_id: mentorId, offering_id: offeringId, status: 'pendente' },
    ...
    if (!error && data) {
      ...
      setBookings(prev => {
        ...
        return existing >= 0
          ? prev.map((b, i) => i === existing ? { ...data, offerings: offering } : b)
  ...
  async function handleCancelBooking(bookingId) {
    ...
    if (!error) setBookings(prev => prev.map(b => b.id === bookingId ? { ...b, status: 'cancelado' } : b))
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
            ...
            {TABS.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                ...
                {t.id === 'agendamentos' && activeBookings.length > 0 && (
                  <span className="ml-1.5 bg-[#1E3A8A] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{activeBookings.length}</span>
                )}
              </button>
            ))}
          </nav>
      ...
      <main className="pb-24 md:pb-8">
          ...
          <div>
            <section className="bg-gradient-to-br from-[#1E3A8A] to-[#2563EB] py-10 md:py-14">
              <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
                <h1 className="text-white text-2xl sm:text-3xl font-bold mb-2">Olá, {firstName}! 👋</h1>
                <p className="text-blue-200 text-sm mb-8">Encontre sua próxima experiência cirúrgica</p>
                <div className="bg-white rounded-2xl p-3 shadow-xl flex gap-2">
                  ...
                  />
                  {searchQuery && (
                    <button onClick={() => setSearchQuery('')} className="px-3 py-2.5 text-[#64748B] hover:text-[#1E293B] text-sm flex-shrink-0">✕</button>
                  )}
                </div>
              </div>
            </section>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 pb-16">
              ...
              {!loadingOfferings && (
                <p className="text-sm text-[#64748B] mb-5">
                  {filteredOfferings.length} {filteredOfferings.length === 1 ? 'mentoria disponível' : 'mentorias disponíveis'}
              ...
              {loadingOfferings ? (
                ...
                <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-[#CBD5E1]">
                  ...
                  <p className="font-semibold text-[#1E293B] mb-1">Nenhuma mentoria encontrada</p>
                  <p className="text-sm text-[#64748B] mb-5">Tente outros termos ou remova os filtros.</p>
                  <button
                    onClick={() => { setSearchQuery(''); setFilterSpec('Todos') }}
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredOfferings.map(o => {
                    ...
                    const existingBooking = bookings.find(b => b.offering_id === o.id && b.status !== 'cancelado')
                    const isApplying = applying === o.id
                    const mentorInitials = (o.profiles?.nome ?? 'M').split(' ').map(w => w[0]).slice(0, 2).join('')
                    return (
                      <div key={o.id} className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                        {o.procedures && (
                          <div className={`px-4 py-2 text-xs font-semibold flex items-center justify-between ${cx?.strip ?? 'bg-[#F8FAFC] border-b border-[#E2E8F0] text-[#64748B]'}`}>
                            <span>{o.procedures.especialidade}</span>
                            {cx && <span className={`px-2 py-0.5 rounded-full ${cx.color}`}>{cx.label}</span>}
                          </div>
                        )}
                        <div className="p-5 flex flex-col flex-1">
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <p className="text-xs font-medium text-[#374151] truncate">{o.profiles?.nome ?? 'Especialista'}</p>
                          </div>
                          <div className="text-xs text-[#64748B] space-y-0.5 mb-4 flex-1">
                            <p>📅 {fmtDate(o.inicio)} · ⏰ {fmtTime(o.inicio)} – {fmtTime(o.fim)}</p>
                            <p>📍 {o.cidade}{o.local_descricao ? ` · ${o.local_descricao}` : ''}</p>
                            <p>👥 {o.max_vagas} {o.max_vagas === 1 ? 'vaga' : 'vagas'}</p>
                          </div>
                          <div className="flex items-center justify-between mb-3">
                            <span className="font-bold text-[#1E3A8A]">R$ {Number(o.preco).toLocaleString('pt-BR')}</span>
                            <button
                          ...
                          {existingBooking ? (
                            <div className={`text-center py-2.5 rounded-xl text-sm font-medium ${BOOKING_STATUS[existingBooking.status]?.color ?? 'bg-gray-100 text-gray-600'}`}>
                              ✓ {BOOKING_STATUS[existingBooking.status]?.label ?? existingBooking.status}
                            </div>
                          ) : (
                            <button
          ...
          <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
            ...
            {activeBookings.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-[#CBD5E1]">
                ...
                <p className="font-semibold text-[#1E293B] mb-1">Nenhum agendamento ativo</p>
                <p className="text-sm text-[#64748B] mb-5">Candidate-se a uma mentoria para começar.</p>
                <button onClick={() => setActiveTab('explorar')} className="px-5 py-2.5 bg-gradient-to-r from-[#1E3A8A] to-[#2563EB] text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity">
                  Explorar Mentorias
                </button>
              </div>
            ) : (
              <div className="space-y-4">
                {activeBookings.map(b => {
                  ...
                  const mentorInitials = mentorNome.split(' ').map(w => w[0]).slice(0, 2).join('')
                  return (
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            <p className="font-semibold text-[#1E293B] text-sm">{mentorNome}</p>
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${bst.color}`}>{bst.label}</span>
                          </div>
                          <p className="text-sm text-[#374151] mb-1">{o?.procedures?.nome ?? o?.titulo ?? '—'}</p>
                          {o?.inicio && <p className="text-xs text-[#64748B]">📅 {fmtDate(o.inicio)} · ⏰ {fmtTime(o.inicio)}</p>}
                          {o?.cidade && <p className="text-xs text-[#64748B]">📍 {o.cidade}{o.local_descricao ? ` · ${o.local_descricao}` : ''}</p>}
                          {o?.preco != null && <p className="text-sm font-bold text-[#1E3A8A] mt-1">R$ {Number(o.preco).toLocaleString('pt-BR')}</p>}
                        </div>
                      </div>
        ...
        {activeTab === 'historico' && (
          <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
            ...
            {pastBookings.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-[#CBD5E1]">
                <p className="text-3xl mb-3">📚</p>
                <p className="font-semibold text-[#1E293B] mb-1">Nenhuma experiência ainda</p>
                <p className="text-sm text-[#64748B]">Seu histórico de mentorias aparecerá aqui.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {pastBookings.map(b => {
                  const o = b.offerings
                  const mentorNome = b.mentor?.nome ?? 'Especialista'
                  const bst = BOOKING_STATUS[b.status] ?? BOOKING_STATUS.concluido
                  const mentorInitials = mentorNome.split(' ').map(w => w[0]).slice(0, 2).join('')
                  return (
                    <div key={b.id} className="bg-white rounded-2xl border border-[#E2E8F0] p-5 shadow-sm">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-[#374151]">{o?.procedures?.nome ?? o?.titulo ?? '—'}</p>
                          {o?.inicio && <p className="text-xs text-[#64748B] mt-0.5">📅 {fmtDate(o.inicio)}</p>}
                          {o?.preco != null && <p className="text-sm font-bold text-[#1E3A8A] mt-1">R$ {Number(o.preco).toLocaleString('pt-BR')}</p>}
                        </div>
                        {b.status === 'concluido' && (
                          <div className="flex flex-col gap-1.5 flex-shrink-0">
                            <button onClick={() => setCertBooking(b)}
          ...
          <div className="max-w-lg mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-sm mb-4">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <p className="font-bold text-[#1E293B] text-lg">{displayNome || '—'}</p>
                  <p className="text-sm text-[#64748B]">{nivelLabel} · {displayCidade || '—'}</p>
                </div>
              </div>
              <div className="space-y-0">
                {[
                  { label: 'Especialidade',   value: (profileOverride.especialidade ?? studentProfile.especialidade) || '—' },
                  { label: 'Ano de formação', value: (profileOverride.ano_formacao  ?? studentProfile.ano_formacao)  || '—' },
                  { label: 'CRM',             value: (profileOverride.crm           ?? studentProfile.crm)           || '—' },
                ].map(f => (
              ...
              <button onClick={() => setShowEditProfile(true)} className="mt-5 w-full py-2.5 border-2 border-[#1E3A8A] text-[#1E3A8A] rounded-xl text-sm font-semibold hover:bg-[#EFF6FF]">
                Editar Perfil
              </button>
            </div>
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'Experiências', value: pastBookings.filter(b => b.status === 'concluido').length },
        ...
        {activeTab === 'docs' && (
          <div className="max-w-lg mx-auto px-4 sm:px-6 py-8">
            ...
            <p className="text-sm text-[#64748B] mb-6">Envie seus documentos para verificação do perfil. PDF ou imagem (JPG, PNG), máx. 10MB.</p>
      ...
      {certBooking && <CertificateModal booking={certBooking} studentName={displayNome} onClose={() => setCertBooking(null)} />}
      ...
      {showEditProfile && (
        <EditStudentProfileModal
          ...
          onSaved={form => {
            setProfileOverride({ nome: form.nome, cidade: form.cidade, nivel: form.nivel, especialidade: form.especialidade, crm: form.crm, ano_formacao: form.ano_formacao })
